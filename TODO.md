...
### 0.484 地形總目錄預註冊（2026-08-02）——先具名再逐一測
D1-D3 已定案（獵取情境/前方距離/背後距離，最終關全過）。待測七維，預測凍結：
- [ ] **D4 牆的等級**：預測 上週>昨日>波段>時段（越公眾的牆越硬）
- [ ] **D5 池子密度**：前方 3 ATR 內池數，預測 密集帶扣分遞增（背後版一併）
- [ ] **D6 翻轉位**：4-24h 內剛被掃的價位當支撐，預測 ≈ 未掃池的墊背效果
- [ ] **D7 區間位置**：雙側合成 0-1，預測 貼撐開多好/貼牆差；須驗對 D2/D3
      的殘餘資訊，無則併入
- [ ] **D8 獵取風暴**：24h 掃池數，兩派預測打架（重置=差 vs 波動=好），無傾向
- [ ] **D9 彈簧後開槍**：最近獵取為深∧收回後的順向訊號，預測 順獵取最肥子集
- [ ] **D10 牆齡**：傾向無效（池齡已死過），最後補完整性
協議（不臨場改）：單維兩半同向 → D1-D3 桶內殘餘檢定 → 最終統計關（置換+
bootstrap+逐季）→ 三關全過才進地形標記/檔位。766 筆同一樣本，~20 looks
