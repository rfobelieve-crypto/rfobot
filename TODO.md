...
- [ ] **S2 結構事件距離**：距最近 BOS（掃擺動高低且守住＝獵取線 ¬R on
      swing）/ CHoCH（趨勢後首次反向破）/ 假BOS（＝R＝獵殺）幾根。預測：
      CHoCH 後順新結構最好；假BOS 後順原結構好（D1 的結構語言版）
- [ ] **D4 牆的等級**：預測 上週>昨日>波段>時段（越公眾的牆越硬）
- [ ] **D6 翻轉位**：4-24h 內剛被掃的價位當支撐，預測 ≈ 未掃池的墊背效果
- [ ] **D9 彈簧後開槍**：最近獵取為深∧收回後的順向訊號，預測 順獵取最肥子集
- [ ] **D5 池子密度**：前方 3 ATR 內池數，預測 密集帶扣分遞增（背後版一併）
- [ ] **D8 獵取風暴**：24h 掃池數，兩派預測打架，無傾向
- [ ] **D10 牆齡**：傾向無效（池齡已死過），最後補完整性
