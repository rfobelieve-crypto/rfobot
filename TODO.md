...
### 0.42 策略 #4「網格系統」規格凍結（2026-08-05）— 待建 shadow 記帳
雙向逆勢加碼 DCA（斐波那契階梯），來自使用者早年實跑過的參數表。
**完整規格與預註冊 Gate 在 `../jarvis/網格系統_規格.md`**（那份是真相源）。
...
- [ ] **P0 shadow 記帳**：照 `research/sweep_failure/shadow_engine.py` 同一模式
      （凍結函式 + 前瞻 CSV + `first_seen_utc`），**必須直接呼叫 `lib-dca.js`
      的 `runDcaBoth`**，不得另寫一份，否則 shadow 與 Gate 會漂移
