...
- [ ] **S1 結構方向**：HH+HL 上升／LH+LL 下降／混合盤整（±10 樞紐，全系統
      同參數）。兩派並列：教科書「順結構好」 vs V7 均值回歸性格「可能反轉」
      ——無傾向注記，資料裁決
- [ ] **S3 折價/溢價**：dealing range＝最近確認主要擺動高↔低，位置 0-1。
      預測 UP 在折價區(<0.5)好、溢價差；DOWN 鏡像。（原 D7 併入取消）
