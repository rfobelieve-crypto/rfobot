...
- [ ] **淨偏斜基線/假象檢查（2026-07-17 登記，優先於融合判斷器）**：手機動態圖觀察到
      現貨淨偏斜綠柱長期偏高（賣側淨撤離結構性 > 買側）。兩個候選解釋要分辨：
      (A) 量測假象——收集器可見窗隨價格遷移，價位滑出窗被機械計成撤單；資料只有
      一週且全程陰跌，遷移方向恰好系統性製造「賣側撤」假象；(B) 真結構偏差
      （現貨簿兩側翻動率天生不對稱，同毛偏斜 +0.06 那族，去均值即可）。
      檢查法：①淨偏斜按小時漲/跌/橫盤分組——綠度跟著下跌時段走→A、橫盤也綠→B；
      ②現貨 vs perp 獨立簿對照——同綠→B、只現貨綠→查窗口邏輯；
      ③A 成立則 F1 判決有污染風險（窗口遷移 vs 動能的假相關），8/10 前必須先解；
      ④watcher `|淨偏斜|≥0.30` 是原始值門檻，口音使 UP/DOWN 觸發率天生不對稱。
      通用解法：淨偏斜改滾動去均值/z-score（同 V7 rolling percentile 哲學）
- [ ] **撤單狀態機判斷器（2026-07-17 登記，依賴上一條先完成）**：把 shock/毛/淨/
      ...
      監控圖頂狀態緞帶（每分鐘一格）+ TG /cancelstate 一行 + 告警照舊。
      判讀卡從三步簡化為「看緞帶顏色」
