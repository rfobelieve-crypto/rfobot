...
自曝檢查（儀器對不對，跟判決分開）
    ...
    S2  **無停損**下，隨機時點的多、空平均相加必須 = 0（算術恆等式：
        R_long = (close−ent)/A、R_short = −(close−ent)/A，逐筆相消）。

        S2 的第一版寫錯了先驗（2026-09-10，留檔不改寫）：原本寫的是
        「B2 在零成本下應貼近零，顯著非零就查儀器」。實測 B2 @480 =
        **+0.0781、CI [+0.0082,+0.1548] 不含零**，照規矩去查儀器，
        結果**儀器是對的、預期是錯的**：

            停損 3 ATR   多 +0.0316  空 +0.0206  相加 +0.0522
            無停損       多 −0.0014  空 +0.0014  相加 +0.0000

        停損把多單的左尾截在 −3 ATR，而空單在同一段路徑上拿到完整的
        右尾；兩個方向都這樣，所以**有停損時「純隨機」的基線本來就是正的**。
        這是停損的算術，不是市場漂移，也不是抽樣偏差。

        後果：**A 臂的毛利裡有一塊是「停損紅利」而不是訊號**，本檔把它
        單獨報成 B2。而 A−B1 的配對差不受它影響（兩臂同樣的停損）。

樣本外（CLAUDE.md 核心原則 9：樣本外放主句，樣本內只在括號裡）
    以事件日中位數切前後半，後半＝樣本外。判準只看後半。
