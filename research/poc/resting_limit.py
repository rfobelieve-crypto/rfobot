...
"""
========================================================================
2026-09-07 **Q1 CONFIRMED-樂觀。凍結利潤全來自「誠實掛單拿不到」的那 12%。**
========================================================================
儀器算術逐項對得上（先查再信，因為數字大到不合理）：
    R_frozen 全體 +0.0367  =  0.8768 × (−0.0504)  +  0.1232 × (+0.6566)
    成交(δ=0)  6,176 筆 (87.68%)  R_frozen 均值 **−0.0504**
    未成交       868 筆 (12.32%)  R_frozen 均值 **+0.6566**

**凍結引擎 +0.0367 的全部利潤，來自那 12.3%——而那正是誠實掛限價永遠不會
成交的那些。** 掃單 bar 收在價位另一側時，凍結引擎照樣記「成交在價位」＝
賣在市價之上，只有掛單等得到；而價格一去不回（做空最賺的情況）就永遠等不到。

處境分解
    MARKETABLE  n=2,865  R_frozen −0.0425  誠實成交率 100%
    RESTING     n=4,179  R_frozen +0.0910  誠實成交率 79.2%   <- 利潤全在這堆

判準結果
    Q1  配對差 **−0.0122  CI [−0.0191, −0.0056]  CONFIRMED-樂觀**
        但配對差**低估**——它只算成交的那些。
    誠實執行下的完整凍結規則（**含 3.5 ATR 災難停損**，本檔的 R_{tag} 就是它）：
        成交子集 meanR **−0.0626  [−0.0889, −0.0386]**
        每事件期望 0.8768 × (−0.0626) = **−0.0549 R**   （凍結 +0.0367）
    Q2  **不可解讀**：Q3 安慰劑 PASS(+0.6889)，但本檔分層變數自己的
        Spearman 是 **−0.7127**，絕對值一樣高只是反號 —— 依本檔自訂的
        「這個不應該高」，Q2 的分級比較同樣被污染。（Q2 本來就 INCONCLUSIVE。）
    Q1 **不依賴**該分層變數（只看成交/未成交），故 Q1 站得住。

與既有結果一致
    `entry_decomp`：A 臂（掛在價位）+0.0272 vs B 臂（觸價那分鐘收盤，實際
    可得）−0.0295。**兩條獨立的誠實執行模型都是負的。**

**未解決的偏差（明寫，不得忽略）**
    MARKETABLE 那一臂在**掃單 bar** 就成交（f_bar = j），而凍結規則是等
    回踩才進場。所以那一臂不是凍結規則的忠實重述，是「掛單當下就被吃掉」
    的實況。它的出場窗也因此不同。這讓「誠實執行 = −0.055」帶著一個
    時點差異，**不是凍結規則的乾淨重算**。要完全乾淨，需要對 MARKETABLE
    另立一個「等回踩才下單」的臂——未做。
