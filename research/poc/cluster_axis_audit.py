...
"""
========================================================================
2026-09-07 **結論：日聚類足夠。價格軸的冗餘對 CI 的影響 < 1%。**
========================================================================
第一版的 C1（比 SE(L)/SE(D)）**回答不了它宣稱要回答的問題**，而 C3 對照組
當場抓到：真實 0.59x、隨機重排 0.60x —— 幾乎一模一樣，所以那個比值量的是
**群數**（價位群 5,988 個 vs 日 919 個，群多重抽細、SE 自然小），不是相依
結構。同族第四次：用一個對目標現象免疫的量去檢查。

改用不受群數影響的組內相關（ICC）與設計效應：

    原始 R · 真實價位群    ICC +0.0925  m̄ 1.18  DEFF 1.0163  SE 膨脹 +0.81%
    原始 R · 隨機對照      ICC +0.0039  m̄ 1.04  DEFF 1.0002  SE 膨脹 +0.01%
    日內去均值 · 真實      ICC +0.0885  m̄ 1.18  DEFF 1.0156  SE 膨脹 +0.78%
    日內去均值 · 隨機對照  ICC +0.1321  m̄ 1.04  DEFF 1.0048  SE 膨脹 +0.24%

價格軸的相依**是真的**（ICC 0.093 對照 0.004），但群太小：**72.7% 的交易是
單筆群、最大的群 6 筆**，所以 DEFF ≈ 1.016，SE 膨脹 < 1%。

**日內去均值那一列的對照壞掉**（隨機重排的 ICC 反而比真實高）——群大小 1.04
幾乎全單筆時 ICC 估計不穩定，故不引用去均值版的 ICC 數值。結論不受影響：
SE 膨脹在四格裡都 < 1%，那是由群大小決定的，與 ICC 估多少無關。

-> **Gate F 與這條線既有的所有 CI 不需要重算。**
