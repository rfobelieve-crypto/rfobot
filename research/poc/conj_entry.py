...
===========================================================================
    ...
    X3 已知答案的對照（**必須**過，否則以下不解讀）
       P0 必須重現 `conj_causal.py` 因果門檻版**事件端**的 **+0.2779**
       （容差 0.02）。重現不出來代表我又寫了第二份實作而且它們不同意。

       **2026-09-07 修正（第一版寫錯，留檔）**：第一版拿 `TRIAGE.md` 的
       **+0.3485** 當參考，但那是配對**差值**（事件 − 對照），而本檔量的是
       **水準**（事件端的原始報酬）。兩者是不同的量。§0.57b 白紙黑字寫著
       「**配對設計對差值有效、對水準無效**」，而我拿差值當水準的參考值 ——
       同一族第五次，且這次是我自己寫進判決檔的那條規矩。
       這是**修正參考量**（比較的東西本來就該是水準），不是放寬容差。
       第一版跑出 P0 +0.2927 vs +0.3485 差 0.0558 判 FAIL，該判決作廢。
    X4 衰減速度（本檔真正要回答的）
...
def main():
    ...
    print("=== 預註冊判準 ===")
    print()
    ref = 0.2779      # conj_causal 事件端水準（見檔頭 X3 修正）
    p0 = res["paths"]["P0 close(t) **不可實現**"]["mean"]
    ok3 = abs(p0 - ref) < 0.02
    print(f"X3 已知答案對照：P0 {p0:+.4f} vs conj_causal 事件端 {ref:+.4f}"
