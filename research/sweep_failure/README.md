...
## 跨市場擴大（2026-07-29，16 個市場，凍結規則零調整）

清單機械化宣告於執行前（各大類最流動的 CME/ICE 期貨 + FX majors），全部報告：

| 資產類別 | 市場 | gross 為正 | 組內平均 net t |
|---|---|---|---|
| 股指 | NQ ES YM RTY NIKKEI | 5/5 | +1.73 |
| 工業/能源 | COPPER CRUDE NATGAS | 3/3 | +1.75 |
| FX | EUR JPY GBP AUD | 4/4 | +1.13 |
| 債券 | UST10Y UST30Y | 2/2 | +0.62 |
| 貴金屬 | GOLD SILVER | 1/2 | −0.41 |

**16 個市場中 15 個 gross 為正**（唯一失敗：黃金）。跨 5 個資產類別
Stouffer 合併 **t = +2.15，完全獨立於 crypto 樣本**。單一市場最強：
NATGAS net t +3.39、RTY +2.86、AUDUSD +2.71、ES +2.21、NQ +2.04。

**為什麼這是本案最重要的證據**：Deflated Sharpe（見
`dsr_and_confseq.py`）顯示 crypto 樣本在扣掉本 repo 17 次試驗的選擇偏誤
後只剩 55% 可信度，且 MinTRL ≈ 450 年——**同一批資料再多也證明不了**。
跨市場重現是 DSR 框架**結構上看不到**的證據類型：一個在 crypto 上被
資料探勘出來的假象，不可能同時出現在股指、能源、外匯、債券上。這把
#3 從「可能是 crypto 資料探勘」重新定位成「**跨資產的結構性微結構現象，
crypto 只是其中一個表現場所**」。

註記：組內高度相關（NQ/ES/YM/RTY ~0.9），所以用組內平均而非逐市場合池
（合池 t=+5.10 有灌水，勿引用）；黃金一致失敗（兩半皆負）反而增加可信度
——全部通過才可疑。Gate F 不變（crypto forward 照走）。
