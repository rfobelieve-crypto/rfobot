...
| `HL_RECORD_TOKEN`（讀 hl-record 服務的唯讀檔案端點＋一次性 /seed） | Railway 服務 `hl-record` ＋ 本機 `flow_system/.env`。**2026-09-15 新增**。外洩後果：讀得到公開鏈上資料的整理版；volume 已種過，/seed 永遠回 409。**輪替時兩邊一起換** |
