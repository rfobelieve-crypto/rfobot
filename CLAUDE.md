...
| **檔案庫 96 篇的 alpha 菜單（哪些做過、哪些沒碰、資料在不在手）** | `docs/alpha_menu.md`（2026-09-13；**只有 5 篇變成過程式碼**）|
...
| 2026-09-13 | **HMM 小額實盤（第 9 次 override）**——單一市場、每所 $50、掛單 Lighter 對沖 HL | **生效到 record-only；翻 live 要再確認一次** |
...
## HMM 小額實盤（2026-09-13，第 9 次 informed override）—— **全文在 `../arb/docs/HMM.md`**

2026-09-15 使用者：「把HMM相關的東西都移過去」。這一節的全文（授權範圍、推翻了
哪一條判準、寫死的紀律、代價自負、上線前的現場數字）**一字未改地搬到 arb**，
HMM 從那邊開 session。上面索引表那一列留著，因為索引才是地圖。
...
| 4 | **兩場館套利**（獨立線，2026-08-28）｜上線構造 = **HMM 對沖做市** | §0.75 家族只錄不做且已 NO-GO；HMM 第 9 次 override 小額實盤中（**全部在 `../arb`**：`docs/HMM.md`、`TODO.md` §1.40–§1.42，2026-09-15 搬過去） | 是（獨立帳戶，每所 $50） |
...
| 錄什麼 | 怎麼錄 | 可回填？ |
|---|---|---|
| 逐地址部位、清算價、觸發單 | 每小時 REST，2,480 地址。**2026-09-15 起在 Railway `hl-record`**，本機 :45 拉回 | **否**（無歷史端點） |
| L2 簿口（20 檔，帶每檔張數 `n`） | 每小時 REST（同上，Railway） | **否** |
| 全市場成交帶（234 幣，雙方地址） | 常駐 WS。**2026-09-15 起錄製器在 `../arb/recorders/hl_tape.py`** | **否** |
| **分鐘級中價與佇列**（前 40 名） | 常駐 WS，牆鐘 60 秒取樣。**2026-09-15 起在 `../arb/recorders/hl_mid.py`** | **否** |

> **2026-09-15 搬家（使用者：「四支全搬」）**：HL 兩支與 Lighter 兩支 WS 錄製器
> （`hl_tape`、`hl_mid`、`lighter_tape`、`lighter_mid`）搬到 `../arb/recorders/`，
> 看門狗是 arb 的 `ops/recorder_watchdog.ps1`（排程 `Arb_RecorderWatchdog`，60 分鐘內最多重啟 3 次）。
> **D: 上的資料路徑一個字沒變**，本 repo 的研究照樣讀 `D:/flowbot_data/{hl,lighter}/...`；
> 旗標改在 `../arb/results/*_last.json`，本 repo 的看板、`accum_live`、`onchain_publish` 經 `arb_home` 讀。
> 理由：那四份資料是 HMM 工具在讀的，而 arb 不准讀 flow_system 產出的東西。
...
>
> **⚠ 2026-09-12 同日稍晚：上面那段的「零費率」被推翻了。**
> **我們的帳戶是 PREMIUM，taker 2.80 bps / maker 0.40 bps，不是 0。**
>
> **(1) 我今天讀錯兩次，過程留檔因為它比結論有用：**
>
> | 讀法 | 依據 | 結論 | |
> |---|---|---|---|
> | 一 | LIT 頁面 `Taker 0.028% / Maker 0.004%` | 「零費率被推翻」 | 錯（以為是逐市場）|
> | 二 | BTC 頁面 `0%/0%` ＋ API 245 市場全 `0.0000` ＋ 文件 Standard 0/0 | 「沒被推翻」 | **也錯** |
> | 三 | BTC 頁面**在帳戶資料載入後**也是 `0.028%` ＋ portfolio 徽章 **PREMIUM** | 真的被推翻 | 對 |
>
> **第二讀錯在哪**：那張 BTC 截圖的 `Available to Trade` 是 **`-`**、
> 右下角還有 rate limited 提示 —— **帳戶資料根本沒載進來，面板顯示的是預設值**。
> 我拿一個沒載入的畫面當證據，而它剛好說了我想聽的話（mistake.md 2026-07-28：
> 符合預期的答案最危險，因為它會讓你停止搜尋）。
>
> **第二個陷阱要單獨寫下來**：`/api/v1/orderBooks` 的 `taker_fee`/`maker_fee`
> 在 **245 個市場上全是 `0.0000`** —— 那是**協定層的市場參數**，
> **不是帳戶要付的錢**。**Lighter 計價的是帳戶，不是市場。**
> 以後不可以從那個欄位讀場館成本。
>
> **(2) 分級表（費率與延遲是綁在一起賣的）**：
>
> | 帳戶級別 | Maker | Taker | Taker 延遲 | Maker 延遲 | **取消延遲** |
> |---|---|---|---|---|---|
> | Standard（預設，**我們不是**） | 0 | 0 | 300ms | 200ms | **300ms** |
> | **Premium（零質押）—— 我們就是這一列** | **0.40 bps** | **2.80 bps** | 140ms | — | **不加延遲** |
> | Premium（質押 500k LIT，−30%） | 0.28 | 1.96 | 140ms | — | 不加延遲 |
> | Plus（要 opt-in） | **0.5 bps 雙邊** | 0.5 bps | 300ms | 200ms | 200ms |
>
> 那 300ms **是交易所刻意加上去的，不是網路**。Premium 那一列的原文是
> 「all order cancelations & Post Only order placements not subject to any
> additional latency」—— 也就是說 **Standard 的免費是用「撤不快」換的**。
>
> **這件事直接改寫 §1.28 那三個門檻的意思。** 原本寫的是
> 吃單 3.00 / naive 掛單 8.27 / **倉位型做市 ≈ 0.5**，而「Lighter 零費率」
> 看起來像是把分母歸零。**它沒有歸零，它換了形式**：
>
> **(3) 對上面那張半價差表的影響：跟 Binance 的差距幾乎沒了。**
> 舊結論寫的是「費用是 0 而不是 3.00」。真實是 **2.80 vs Binance 的 3.00**
> —— **差 0.20 bps，不是差 3.00**。§1.28 的吃單門檻因此幾乎不動：
> 毛利 +0.58 對門檻 2.80，**仍然吃不動**。
>
> 真正改變的是**掛單那一邊**：Lighter 的 maker 是 **0.40 bps**，
> 而 §1.28 算出的倉位型做市門檻是 **≈ 0.5**。**兩個數字坐在同一條線上並非巧合**
> —— 場館把「可以做市的權利」明碼標價賣給你，Premium 買的就是
> 「撤單不被加延遲」，而那正是 Market Making For Dummies 說的那個最重要的數。
>
> **(4) 「改回 Standard 拿 0/0」這個選項不存在（2026-09-12 稍晚更正）。**
> 使用者提供的分級對照有一行我漏掉的：**Standard 的 API rate limit 約
> 60 req/min，Premium 是 sendTx 4,000~48,000/min（依質押）**。
> 60 次/分 = **1 次/秒**，而我們量到的取消往返是 50 ms ——
> **Standard 在結構上做不了做市**，跟「300ms 值幾 bps」無關。
> **0.40 bps 是入場費，不是可選成本。** 官方的分類也這麼寫：Standard 的
> 適合對象是零售／中低頻／方向性／Funding Arb，Premium 才是 HFT／做市。
>
> 真正的成本槓桿是**質押 LIT**（0.40 -> 0.28 bps，−30%），而那是
> **競爭均衡價不是優勢** —— 逐筆成交帶量到 **69.9% 的成交額已經在 0.28**。
> 詳見 TODO §1.31 與 §1.33。
>
> **⚠⚠ 2026-09-12 最後一次更正，而這一次改變的是策略方向不只是數字。**
> 上面那段（以及 §1.33 的第一版）說「Lighter 上沒有人付 0 費率」。**錯了。**
> **零費率是用「不寫那個欄位」表示的**，而我數的是 `== 0`。
>
> 重驗（53,784 筆一般成交，測「同一帳戶會不會有時有欄位有時沒有」）：
> 吃單方 1,545 個帳戶裡**只有 2 個混合**、掛單方 364 個裡**0 個混合**
> -> 帳戶屬性不是資料缺漏。佐證：有欄位的那群裡**沒有一筆是 0.000**
> （0.50 Plus 與 1.96–2.80 Premium 階梯全在）。
>
> | | 零費率（Standard） | 有付費 |
> |---|---|---|
> | **吃單** | **74.9% 的成交額** | 加權 1.919 bps |
> | 掛單 | 11.4% 的成交額 | 加權 0.298 bps |
>
> **所以 §1.28 的「吃單門檻 3.00」在 Lighter Standard 上是 0 ＋ 價差。**
> 而同日量到 Lighter 最緊的市場半價差是 **0.01–0.02 bps**（BTC / XAG / BNB）。
> 慢策略的來回成本因此是 **約 0.02–0.05 bps**，不是 3.00 ——
> **這是第一次有門檻低於已量到的毛利（+0.58，待在 Lighter 重量）。**
>
> Standard 的代價（300ms、60 req/min）**只擋做市**，不擋小時級再平衡。
> 正確的分工是 **Standard 給慢策略、Premium 給做市**，
> 而我們現在用 Premium 跑慢策略 = 白付 2.80 bps 的吃單費。
> 未量的：Lighter 宇宙上的毛利（09-18）。登記在 TODO §1.34。
>
> **⚑ 2026-09-12 首次實測取消延遲**（`tools/cancel_latency.py --live -n 20`，
> NBIS，post-only 掛在 mid −500 bps、名目 $10.20、六道閘全過、掃尾 0 張殘留）：
>
> | | n | 中位 | p90 | 最大 |
> |---|---|---|---|---|
> | 送單往返 | 20 | 54.1 ms | 67.0 | 126.0（冷 TLS）|
> | **取消往返** | 20 | **49.9 ms** | **54.7** | **60.3** |
>
> 帳戶是 Premium，所以**這就是我們自己的往返延遲，不含場館強加的地板**。
> 連帶兩件事：
> (a) **300ms 是加在 50ms 之上的**（原文 "not subject to any **additional**
> latency"），退回 Standard ＝ 50 -> 約 350 ms、**七倍**，那個選項比原先看起來更貴；
> (b) **取消延遲不是我們的綁束** —— 50 ms 遠低於分鐘級的決策尺度，
> **不需要主機代管**。上面「延遲競賽進不去」那句仍然對（頂檔報價是 microsecond
> 的生意），但**倉位型做市在分鐘尺度上不需要贏那場競賽**，而這是第一次有量測
> 支持這句話，不是推論。
>
> **綁束仍然在毛利那一側**：§1.28 的 +0.58 bps 是在 Binance 量的，
> **Lighter 上的毛利一列都還沒量**（重量日期 09-18）。延遲量得再漂亮，
> 沒有毛利就不構成可交易。
>
> **(5) 下游已重算（同日）**：`arblib/fees.py` 改正 → `arb_cost_model.json` 重生成。
> 每筆來回的費用桶：單腿 Lighter 的配對 **+5.60 bps**（SNDK / NBIS / ANTH /
> BTC / HYPE / ZEC / NEAR），**兩腿都在 Lighter 的 +11.20**（GOLD_LL / NVDA_LL）。
> 只扣費的淨值：NBIS **+6.29 → +0.62**、GOLD_LL **+3.99 → −7.24**、
> NVDA_LL **+1.47 → −9.79**。**除了 ANTH（帶 428 bps，本來就是異常值），
> 全家族只扣費都不再為正。** §0.75／§1.20／§1.21 引用的數字全部要重跑，
> 而它們在 `../arb` 那個 repo。
>
> 上面 (c) 那一條同步更新：零費率**從來就不是文件查證**——文件講的是
> Standard，而我們不在那一級。真正的查證是**帳戶自己的頁面**，
> 而收據查證（`fee_receipts.py` 要帳戶成交）仍然沒做。
...
### HMM（對沖做市）與 shadow 模式退場 —— 全文在 `../arb/docs/HMM.md`

2026-09-13 使用者命名 HMM（Hedged Market Making，**不叫套利**：沒有收斂交易）；
2026-09-14 使用者決定 shadow 退場、只跑實盤。兩節全文 2026-09-15 搬到 arb。
命名紀律照舊：**這是使用者定的詞，我不得自行發明「HMM-A」「HMM+」這類變體名。**
