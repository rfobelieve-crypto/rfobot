...
| **流動性獵取**（策略 #2） | 規則凍結、forward 記帳 | **兩條時鐘並行**：Gate F 正式軌道（變體 A·無濾網·core9）**95/1400**，CI 下緣 **+0.063**、7/9 幣正 —— **兩個質性條件已達標，只差樣本**（~2027-01）；變體 B（＋淺穿越濾網·29幣）**438/1400**，CI 下緣 **−0.016**（~2026-09）。**全部變體的註冊簿在 TODO §0.43**（散在四個檔案曾導致誤判主線）|
| **撤單流**（策略 #3） | 方向性判決 FAIL | `cancel_lead_ic` 判決排程 **2026-08-10**（事件已累積 1007 筆）|
...
FAIL（CI 下緣 51.5% < 52%）一致 —— 進場準度在衰退。而交易層 15 筆勝率只有
