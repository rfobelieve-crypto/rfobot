...
| 2026-09-12 | **SDV 意圖層開啟（`CONJ_INTENTS=1`），消費端只有 paper** | **生效（第 8 次 override，見下）** |
...
## SDV 意圖層開啟（2026-09-12，第 8 次 informed override）—— **消費端只有 paper**

**使用者決定（原話）**：「研究端那邊開 CONJ_INTENTS=1，用 railway cil 去把它設好，
我用戶端那邊處理好了剩這一步」。
...
而且**不會有任何東西報錯**——這是 mistake.md 2026-07-28
「不確定哪個服務跑哪支程式時看 Dockerfile 的 CMD」的同一族，只是這次答案是
「沒有任何服務，它在地端」。

**這條 override 推翻的是什麼（誠實版）**

`conj_watch.py` 的恢復條件區塊（檔內 118–155 行）寫著三條，而第 (b) 條
**在樣本外沒有達成**：

| 口徑 | 每筆 | CI 下緣 | 逐幣 |
|---|---|---|---|
| 全期、delay=3（`conj_redef --decided --sig and`） | +0.3317 | **+0.0714** | 9/9 |
| **樣本外那一半**（主句，核心原則 9） | **+0.1833** | **−0.132** | 9/9 |

**樣本外的 CI 仍然跨零。** 依核心原則 10，一併報的是
**P(優勢>0) = 85.0%**、回落 p50 **43.3%** / p95 59.7% / **P(回落>50%) = 24.1%**
（2x、3 槽、1500 條重抽路徑）。那是一個明確值得考慮的賭注，不是「沒有證據」
——但它**確實沒有通過我自己寫下的恢復條件**，所以這是一次知情的推翻。

**為什麼這次的代價遠低於第 7 次 override**

| | 第 7 次（2026-09-08） | **本次** |
|---|---|---|
| 消費端 | Bitget 真錢、$300-500、全自動 | **只有 paper** |
| 帳戶風險 | 與 V7 共用帳戶（已被手動單爆過兩次） | **沒有帳戶** |
| 「不得進 executor」硬規則 | 明文違反 | **沒有違反**（paper 不是 executor） |

用戶端鎖死在 paper，**而且是寫死的不是設定的**：
`../jarvis/public/u.html:2640` 有一段刻意的註解說明為什麼不吃 `uiMode`，
`u.html:2651` 送 `mode:'paper'`、`../jarvis/src/tenants.js:608` 也用 `mode:'paper'`。
**要變成真錢必須改程式碼，不是改一個旗標**——那道界線請不要拿掉。

**寫死、事後不放寬的紀律（沿用第 7 次那一節，逐條仍然有效）**

- **paper 的損益不得作為 edge 證據。** 時鐘的計分器不吃這批資料。
  這是 §0.92 判掉 C/D 變體的同一種錯（事後挑統計量最好的樣本）。
- **唯一要看的判準是執行面**：意圖價 vs 模擬成交價差幾 bps、
  以及 2 分鐘死線在真實延遲下撐不撐得住（`latency_ms_max`）。
- **`ENTRY_DELAY_MIN` 維持 3，不得放寬到 5。**「最大可用 delay = 5 分鐘」
  靠的是 +0.0003 的 CI 下緣——那不是餘裕，是剛好沒跨零。
- **不得挑幣**（TODO §1.03s）：九幣等權，不為任何單一幣加碼。
- 要進真錢必須另開一次 override，並且先滿足第 7 次那一節的判準
  （實際成交 vs 意圖價 ≤ 2 bps）。

**怎麼知道它真的開了**：判準是**產物**不是旗標——
`research/results/conj_watch.log` 的每分鐘那一行要從
`intents=**HALTED**` 變成 `intents=ON`，而且事件發生時
`conj_intents` 要有新列、`/public/conj-signals` 要撈得到。
**不要看排程的 `LastTaskResult`**（非同步啟動器的退出碼與子行程無關，
mistake.md 2026-09-06）。
...
>
> **⚠ 2026-09-12 追加：那個 3.00 bps 是 Binance 的費率，而我們已經在錄一個
> 零費率的場館。** `arblib/fees.py` 的 `lighter` 是
> `taker_bps 0.0 / maker_bps 0.0`（標 `verified`，出處是 Lighter 文件）。
> 當天從 arb 錄製器的 `minutes.csv` 量 Lighter 腿的頂檔（**176 分鐘、只有當天**）：
>
> | Lighter 標的 | 半價差 | 5 bps 內深度 | 每分鐘成交 |
> |---|---|---|---|
> | **BTC** | **0.42 bps** | **$570,164** | **$9,182** |
> | GOLD_LL (XAU) | 0.08 | $80,295 | $1,747 |
> | ANTH | 1.37 | $9,117 | $1,730 |
> | HYPE | 1.39 | $135,825 | $577 |
> | NBIS / NEAR / NVDA_LL | 0.89 / 3.18 / 0.91 | $22k–136k | **$0** |
>
> Binance 厚標的的半價差是 0.50 —— **Lighter BTC 比它緊，而且費用是 0 而不是 3.00。**
>
> **所以綁束換了人：不是費用、不是價差、不是深度，是流量。**
> NBIS / NEAR / NVDA_LL 每分鐘成交 **$0**（簿口深但沒人交易 ——
> 對做市是餓死、對吃單是沒有逆向流可吃）。**四項同時成立的只有 BTC。**
>
> **這不是「MFT 活了」，是「§1.27 的判決是場館特定的」。** 三件事沒做：
> (a) 毛利 +0.58 是在 Binance 的標的上量的，**必須在 Lighter 重量**；
> (b) 樣本 176 分鐘不是判決，成交帶今天才上線；
> (c) 零費率是**文件**查證不是**收據**查證（`fee_receipts.py` 要帳戶成交，我們沒有）。
> 重量的日期跟 §1.29 同一天（**09-18**），那時會有約 6 天的 Lighter 成交帶。
